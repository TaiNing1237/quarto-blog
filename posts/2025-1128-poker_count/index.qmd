...
// 繪製直方圖
{
  ...
  Plotly.newPlot(div, [{
    ...
    y: handTypes.map(d => d.count),
    marker: { color: '#69b3a2' },
    text: handTypes.map(d => d.count.toLocaleString()),
    textposition: 'auto'
  }], {
    yaxis: {
      title: 'Number of Combinations (log scale)',
      type: 'log',
      gridcolor: '#e5e5e5'
    },
    xaxis: {
...
// 繪製直方圖
{
  ...
  Plotly.newPlot(div, [{
    ...
    y: texasHandTypes.map(d => d.count),
    marker: { color: 'steelblue' },
    text: texasHandTypes.map(d => `${d.count.toLocaleString()}<br>${(d.count / 133_784_560 * 100).toFixed(2)}%`),
    textposition: 'auto',
    hovertemplate: '<b>%{x}</b><br>Count: %{y:,}<br>Freq: %{text}<extra></extra>'
  }], {
    yaxis: {
      title: 'Number of Combinations (log scale)',
      type: 'log',
