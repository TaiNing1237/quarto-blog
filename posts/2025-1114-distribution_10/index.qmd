...
| Name, Notation | Parameters | PDF | Range | Mean | Variance | CF |
|:---------|:--------:|:---------:|:---------:|:--------:|:---------:|---------:|
| **Normal** <br> $\mathcal{N}(\mu, \sigma^2)$  | mean: $\mu$ <br> variance: $\sigma^2$ | $\displaystyle e^{-\frac{1}{2\sigma^2}x^2+\frac{\mu}{\sigma^2}x}$ | $x \in (-\infty, \infty)$ | $\mu$ | $\sigma^2$ | $e^{i\mu t - \frac{1}{2}\sigma^2 t^2}$ |
| **Chi-Squared** <br> $\chi^2(k)$ | degree of freedom (d.o.f): $k$ | $\displaystyle x^{\frac{k}{2}-1}e^{-\frac{x}{2}}$ | $x\in(0, \infty)$ | $k$ | $2k$ | $(1 - 2it)^{-\frac{k}{2}}$ |
| **Chi [Optional]** <br> $\chi(k)$ | degree of freedom (d.o.f): $k$ | $\displaystyle x^{k-1}e^{-\frac{x^2}{2}}$ | $x\in(0, \infty)$ | $\sqrt{2} \frac{\Gamma(\frac{k+1}{2})}{\Gamma(\frac{k}{2})}$ | $k - \left(\sqrt{2} \frac{\Gamma(\frac{k+1}{2})}{\Gamma(\frac{k}{2})}\right)^2$ | ugly | 
| **Exponential** <br> $\mathrm{Exp}(\lambda)$ | rate: $\lambda$ | $\displaystyle e^{-\lambda x}$ | $x\in(0, \infty)$ | $\frac{1}{\lambda}$ | $\frac{1}{\lambda^2}$ | $\frac{\lambda}{\lambda - it}$ |
| **Gamma** <br> $\mathrm{Gamma}(\alpha, \theta)$ | shape: $\alpha$ <br> scale: $\theta$ | $\displaystyle x^{\alpha-1} e^{-\frac{x}{\theta}}$ | $x\in(0, \infty)$ | $\alpha\theta$ | $\alpha\theta^2$ | $\left(1 - i\theta t\right)^{-\alpha}$ |
| **Beta** <br> $\mathrm{Beta}(\alpha, \beta)$ | shape1: $\alpha$ <br> shape2: $\beta$ | $\displaystyle x^{\alpha-1}(1-x)^{\beta-1}$ | $x\in(0, 1)$ | $\frac{\alpha}{\alpha+\beta}$ | $\frac{\alpha\beta}{(\alpha+\beta)^2(\alpha+\beta+1)}$ | ${}_1F_1(\alpha; \alpha+\beta; it)$ |
| **Uniform** <br> $\mathrm{Uniform}(-1, 1)$ | NA | $\displaystyle \text{constant}$ | $x\in(-1, 1)$ | $0$ | $\frac{1}{3}$ | $\frac{\sin(t)}{t}$ |
| **Cauchy** <br> $\mathrm{Cauchy}$ | NA | $\displaystyle \frac{1}{1+x^2}$ | $x\in(-\infty, \infty)$ | undefined | undefined | $e^{-|t|}$ |
| **Student's t** <br> $t(\nu)$ | dof: $\nu$ | $\displaystyle \left(1+\frac{x^2}{\nu}\right)^{-\frac{\nu+1}{2}}$ | $x\in(-\infty, \infty)$ | $0$ for $\nu>1$ | $\frac{\nu}{\nu-2}$, for $\nu>2$ | $\frac{K_{\frac{\nu}{2}}(\sqrt{\nu}|t|) (\sqrt{\nu}|t|)^{\frac{\nu}{2}}}{\Gamma(\frac{\nu}{2}) 2^{\frac{\nu}{2}-1}}$ |  
| **F-distribution** <br> $F(d_1, d_2)$ | dof 1: $d_1$ <br> dof 2: $d_2$ | $\left(\frac{d_1 x}{d_1 x + d_2}\right)^{\frac{d_1}{2}} \left(\frac{d_2}{d_1 x + d_2}\right)^{\frac{d_2}{2}} x^{-1}$ | $x\in(0, \infty)$ | $\frac{d_2}{d_2 - 2}$, for $d_2$ >2 | $\frac{2 d_2^2 (d_1 + d_2 - 2)}{d_1 (d_2 - 2)^2 (d_2 - 4)}$, for $d_2$ >4 | ugly | 
...
| Name, Notation | Parameters | PMF | Range | Mean | Variance | CF |
|:---------|:--------:|:---------:|:--------:|:--------:|:---------:|---------:|
| **Bernoulli** <br> $\mathrm{Bernoulli}(p)$ | success prob.: $p$ | $\displaystyle p^x (1-p)^{1-x}$ | $x\in\{0,1\}$ | $p$ | $p(1-p)$ | $1 - p + p e^{it}$ |
| **Binomial** <br> $\mathrm{Binomial}(n, p)$ | trials: $n$ <br> success prob.: $p$ | $\displaystyle \binom{n}{x} p^x (1-p)^{n-x}$ | $x\in\{0,1,\ldots,n\}$ | $np$ | $np(1-p)$ | $(1 - p + p e^{it})^n$ |
| **Poisson** <br> $\mathrm{Poisson}(\lambda)$ | rate: $\lambda$ | $\displaystyle \frac{\lambda^x e^{-\lambda}}{x!}$ | $x\in\{0,1,2,\ldots\}$ | $\lambda$ | $\lambda$ | $e^{\lambda(e^{it}-1)}$ |
| **Geometric** <br> $\mathrm{Geometric}(p)$ | success prob.: $p$ | $\displaystyle (1-p)^{x} p$ | $x\in\{0,1,2,\ldots\}$ | $\frac{1-p}{p}$ | $\frac{1-p}{p^2}$ | $\frac{p e^{it}}{1 - (1-p)e^{it}}$ |
| **Negative Binomial** <br> $\mathrm{NegBin}(r, p)$ | successes: $r$ <br> success prob.: $p$ | $\displaystyle \binom{x+r-1}{r-1} p^r (1-p)^x$ | $x\in\{0,1,2,\ldots\}$ | $\frac{r(1-p)}{p}$ | $\frac{r(1-p)}{p^2}$ | $\left(\frac{p e^{it}}{1 - (1-p)e^{it}}\right)^r$ |
